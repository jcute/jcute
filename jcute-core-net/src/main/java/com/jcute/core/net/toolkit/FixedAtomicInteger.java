...
public class FixedAtomicInteger{
	...
	public FixedAtomicInteger(int min,int max){
		this.minValue = min;
		this.maxValue = max;
		this.integer = new AtomicInteger(min);
	}
	...
	public final int getAndIncrement(){
		for(;;){
			...
			if(this.integer.compareAndSet(current,next)){
				return current;
			}
		}
	}
	...
	public boolean compareAndSet(int expect,int update){
		return this.integer.compareAndSet(expect,update);
	}
